...
class _UnifiedInventoryPageState extends State<UnifiedInventoryPage> {
  ...
  Future<void> _analyzeImage(XFile imageFile) async {
    try {
      ...
      if (response.recognizedIngredients.isNotEmpty) {
        ...
        _showError(
            'No ingredients recognized in the image. Try taking a clearer photo with better lighting.');
      }
    } catch (e) {
      setState(() {
        _isAnalyzing = false;
  ...
  Future<void> _showRecognitionResults(
      ImageRecognitionResponse response, XFile sourceImage) async {
    await showDialog(
      context: context,
                    ...
                    return Card(
                      child: ListTile(
                        ...
                        subtitle: Column(
                          ...
                          children: [
                            Text(
                                'Category: ${IngredientCategory.getDisplayName(ingredient.category)}'),
                            Text(
                            ...
                            if (ingredient.quantityEstimate != null)
                              Text(
                                  'Quantity: ${QuantityDescription.getDisplayName(ingredient.quantityEstimate!)}'),
                          ],
                        ),
  ...
  Map<String, List<InventoryItem>> get _itemsByCategory {
    ...
    final sortedCategories = itemsByCategory.keys.toList()
      ..sort((a, b) {
        ...
        if (a == IngredientCategory.spirits && b != IngredientCategory.spirits)
          return -1;
        if (b == IngredientCategory.spirits && a != IngredientCategory.spirits)
          return 1;
        if (a == IngredientCategory.liqueurs &&
            b != IngredientCategory.liqueurs) return -1;
        if (b == IngredientCategory.liqueurs &&
            a != IngredientCategory.liqueurs) return 1;
          ...
          if (_stats != null)
            Container(
              ...
              child: Row(
                ...
                children: [
                  _StatItem(
                      label: 'Total', value: _stats!.totalItems.toString()),
                  _StatItem(
                      label: 'Categories',
                      value: _stats!.byCategory.length.toString()),
                  _StatItem(
                      label: 'Expiring',
                    ...
                    items: [
                      ...
                      ...IngredientCategory.all.map(
                        (category) => DropdownMenuItem(
                          value: category,
                          child:
                              Text(IngredientCategory.getDisplayName(category)),
                        ),
                      ),
                    ],
                    onChanged: (value) {
